...
import { expandDrawer, logout, openPublishModal, refreshUserData } from '@/store/actions';
    ...
    <>
      <MotionAppBar
            ...
            {showBreadCrumbs && (
              <Box marginLeft={'1rem'} display={'flex'} alignItems={'center'}>
                ...
                {(checkPermissions(user?.roles || [], 'CLIENT_COLLABORATOR_ADD', story?.id) ||
                  ...
                  <Box width={'5.75rem'} marginLeft={'1rem'}>
                    <MuiButton
                      type='button'
                      disabled={false}
                      loading={false}
       ...
       {/* //   {checkRoleAndPermission(user?.roles, 'Subscriber_Individual', 'CLIENT_STORY_CREATE', user?.id) && ( 
          ...
          {showCancelButton && (
            <Box width={'5.75rem'}>
              <MuiButton type='submit' disabled={false} loading={false} variant={'outlined'} method={handleCloseButton}>
